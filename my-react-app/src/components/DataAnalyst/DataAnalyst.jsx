...
const DataAnalyst = () => {
  ...
  // small derived values for this view
  const rotationStocks = 8.4;
  const delaiPreparation = '1h 15m';
  const tauxErreur = 0.21;
  const remplissageEntrepot = warehouses.length ? Math.round((warehouses.reduce((s,w)=>s+(w.used||0),0) / warehouses.reduce((s,w)=>s+(w.capacity||0),1))*100) : 0;
  const topReferences = [];
